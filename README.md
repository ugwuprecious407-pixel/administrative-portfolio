# Administrative & Virtual Assistant Portfolio
**Name:** Precious Ugwu  
**Role:** Administrative Assistant | Customer Support | Virtual Assistant  

This portfolio contains simulated projects demonstrating real-world administrative tasks using Google Workspace tools.

---

## 📂 Project 1: Email Inbox Management
**Client:** GreenLeaf Consulting (Simulated)

**Skills Demonstrated:**
- Email organization and prioritization
- Professional email communication
- Email action tracking and follow-ups

📁 Folder: /email-inbox-management

---

## 📅 Project 2: Calendar Management & Meeting Scheduling
**Client:** GreenLeaf Consulting (Simulated)

**Skills Demonstrated:**
- Meeting scheduling
- Calendar organization
- Time management

📁 Folder: /calendar-management

---

## 🗂️ Project 3: Google Drive & File Organization
**Client:** GreenLeaf Consulting (Simulated)

**Skills Demonstrated:**
- Folder structuring
- File naming conventions
- Image and digital asset management

📁 Folder: /drive-file-organization

---

## 🛠 Tools Used
- Gmail  
- Google Calendar  
- Google Drive  
- Google Docs & Sheets  

---# administrative-portfolio
Administrative &amp; Virtual Assistant Portfolio – Simulated Projects
